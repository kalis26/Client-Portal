...
export function accountEmail(kind: "reset" | "invite", token: string, locale = "fr") {
  const href = kind === "invite" ? `${appUrl()}/accept-invitation?token=${encodeURIComponent(token)}` : `${appUrl()}/reset-password?token=${encodeURIComponent(token)}`;
  const isFrench = locale === "fr";
  return { href, subject: isFrench ? (kind === "invite" ? "Invitation à votre espace Rezo" : "Réinitialisez votre mot de passe") : (kind === "invite" ? "Invitation to your Rezo workspace" : "Reset your password"), body: isFrench ? (kind === "invite" ? "Créez votre mot de passe pour accéder à votre espace client." : "Utilisez ce lien pour définir un nouveau mot de passe.") : (kind === "invite" ? "Create your password to access your client workspace." : "Use this link to set a new password.") };
}
