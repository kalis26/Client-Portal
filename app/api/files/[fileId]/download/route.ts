...
export async function GET(_: NextRequest, { params }: { params: Promise<{ fileId: string }> }) { const file = await db.query.projectFiles.findFirst({ where: eq(projectFiles.id, (await params).fileId) }); if (!file || !(await mayAccessProject(file.projectId))) return new Response("Forbidden", { status: 403 }); return new Response(await retrieveEncryptedFile(file.storageKey), { headers: { "Content-Type": file.contentType, "Content-Disposition": `attachment; filename="${file.filename.replaceAll('"','') }"`, "Cache-Control": "private, no-store" } }); }
